...
func (a *App) checkWintunDLL() {
	...
	if _, err := os.Stat(dllPath); os.IsNotExist(err) {
		a.log("=================================================================")
		a.log("【严重警告】未检测到 wintun.dll 文件！TUN 模式无法工作")
		a.log("请将 wintun.dll（amd64 版本）放到目录: " + baseDir)
...
func (a *App) onReady() {
	...
	// 持久同步 TUN 状态（最可靠方案）
	go func() {
		a.log("等待 mihomo 启动并初始化 TUN...")
		time.Sleep(4500 * time.Millisecond)

		a.log("开始持久同步 TUN 状态（最多等待 60 秒）...")
		success := a.waitForTunReady(mTun, 60)
		if success {
			a.log("TUN 状态初始同步完成")
		} else {
			a.log("【警告】TUN 状态同步超时，建议手动点击“虚拟网卡”刷新状态")
		}
...
// ==================== 持久等待 TUN 就绪（核心修复） ====================
func (a *App) waitForTunReady(m *systray.MenuItem, maxSeconds int) bool {
	for i := 0; i < maxSeconds*2; i++ { // 每500ms查询一次
...
func (a *App) asyncToggleTun(m *systray.MenuItem, expectedState bool) {
	...
	for i := 0; i < 4; i++ {
		...
		if a.verifyTunState(expectedState, m) {
			a.logf("TUN 切换完成，当前实际状态: %v", a.isTUNEnabled)
			return
		}
	}
	a.syncTunState(m)
...
func (a *App) verifyTunState(expectedState bool, m *systray.MenuItem) bool {
	return a.fetchAndUpdateTunState(m) && a.isTUNEnabled == expectedState
}

func (a *App) syncTunState(m *systray.MenuItem) {
	if a.fetchAndUpdateTunState(m) {
		return
	}
	for i := 0; i < 3; i++ {
